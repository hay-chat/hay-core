...
const renderedContent = computed(() => {
  ...
  content = content.replace(/\[action\]\(([^)]+)\)/g, (match, id) => {
    ...
    return `<span class="mcp-merge-field mcp-action" data-type="action" data-id="${id}" contenteditable="false">⚡ ${name}</span>`;
  });
  ...
  content = content.replace(/\[document\]\(([^)]+)\)/g, (match, id) => {
    ...
    return `<span class="mcp-merge-field mcp-document" data-type="document" data-id="${id}" contenteditable="false">📄 ${name}</span>`;
  });
  
  // Handle empty content placeholder
  if (!content.trim()) {
    return `<span class="placeholder">${props.placeholder || 'Enter instruction...'}</span>`;
  }
...
const convertToMarkdown = (htmlContent: string): string => {
  ...
  // Remove placeholder
  content = content.replace(/<span class="placeholder">.*?<\/span>/g, '');
  
  // Convert action tags back to markdown - more comprehensive regex
  content = content.replace(/<span[^>]*class="[^"]*mcp-action[^"]*"[^>]*data-id="([^"]+)"[^>]*>.*?<\/span>/g, '[action]($1)');
  content = content.replace(/<span[^>]*data-id="([^"]+)"[^>]*class="[^"]*mcp-action[^"]*"[^>]*>.*?<\/span>/g, '[action]($1)');
  content = content.replace(/<span[^>]*mcp-action[^>]*data-id="([^"]+)"[^>]*>.*?<\/span>/g, '[action]($1)');
  
  // Convert document tags back to markdown - more comprehensive regex
  content = content.replace(/<span[^>]*class="[^"]*mcp-document[^"]*"[^>]*data-id="([^"]+)"[^>]*>.*?<\/span>/g, '[document]($1)');
  content = content.replace(/<span[^>]*data-id="([^"]+)"[^>]*class="[^"]*mcp-document[^"]*"[^>]*>.*?<\/span>/g, '[document]($1)');
  content = content.replace(/<span[^>]*mcp-document[^>]*data-id="([^"]+)"[^>]*>.*?<\/span>/g, '[document]($1)');
  ...
  const actionClassSpans = tempDiv.querySelectorAll('span.mcp-action');
  ...
  const documentClassSpans = tempDiv.querySelectorAll('span.mcp-document');
  ...
  content = content.replace(/<span[^>]*class="[^"]*mcp-merge-field[^"]*"[^>]*>([^<]+)<\/span>/g, '$1');
...
const handleKeyDown = (e: KeyboardEvent) => {
  ...
  if (target.classList?.contains('mcp-merge-field')) {
    e.preventDefault();
    return;
  }
};
...
const handleFocus = () => {
  ...
  // Hide placeholder on focus if content is empty
  const editor = editorRef.value;
  if (editor && !props.modelValue.trim()) {
    editor.innerHTML = '';
  }
};
...
const insertReference = (type: 'action' | 'document', item: any) => {
  ...
  if (slashIndex !== -1) {
    ...
    tagElement.className = `mcp-merge-field mcp-${type}`;
    tagElement.setAttribute('data-type', type);
    tagElement.setAttribute('data-id', item.id);
    tagElement.contentEditable = 'false';
    tagElement.innerHTML = type === 'action' ? `⚡ ${item.name}` : `📄 ${item.name}`;
    ...
    while (currentNode = walker.nextNode()) {
      ...
      if (currentOffset + nodeLength > slashIndex) {
        ...
        const afterQuery = currentNode.textContent?.slice(nodeSlashIndex + 1 + (cursorOffset - slashIndex - 1)) || '';
...
watch(() => props.modelValue, (newValue) => {
  ...
  if (editor && document.activeElement !== editor && !isConverting) {
    ...
    if (!newValue.trim()) {
      editor.innerHTML = `<span class="placeholder">${props.placeholder || 'Enter instruction...'}</span>`;
    } else {
      // Update visual content to show tags when not focused
