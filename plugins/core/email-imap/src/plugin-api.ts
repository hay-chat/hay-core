...
export class PluginApiClient {
  ...
  /**
   * Call a tRPC query on the plugin API.
   * tRPC queries use GET with URL-encoded JSON input.
   * Response format: { result: { data: { ...output } } }
   */
  async query<T>(procedure: string, input: Record<string, unknown>): Promise<T> {
    const encodedInput = encodeURIComponent(JSON.stringify(input));
    const url = `${this.apiUrl}/v1/pluginApi.${procedure}?input=${encodedInput}`;
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Plugin API query failed: ${response.status} ${errorText}`);
    }
